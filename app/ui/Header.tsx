...
export default function Header() {
  ...
  const linkClass = (href: string) => `${isActive(href) ? "!text-[#061D1B] font-medium" : "!text-[#697775]"}`;
            ...
            <div className="flex flex-col gap-4">
              {navItems.map((item) => (
                <Link
                  ...
                  className={`py-2 px-4 border-b border-b-[#d5d5d5] hover:bg-gray-100 transition ${isActive(item.href) ? "text-[#061D1B] font-semibold" : ""}`}>
                  {item.label}
                </Link>
              ))}
            </div>
